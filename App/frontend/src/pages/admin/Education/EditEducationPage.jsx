...
const EditEducationPage = () => {
  ...
  useEffect(() => {
    const loadEducation = async () => {
      if (!id) {
        showToast('error', 'Education ID is required');
      ...
      try {
        ...
        showToast('error', err.response?.data?.message || 'Failed to load education');
        navigate('/admin/education');
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:5000/api/education/${id}`, formData, {
        headers: {
      ...
      showToast('success', response.data.message || 'Education updated successfully');
      navigate('/admin/education');
    } catch (err) {
      console.error('Error updating education:', err);
      showToast('error', err.response?.data?.message || 'Failed to update education');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className={`mt-2 text-lg ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
                Update your academic achievements and qualifications
              </p>
            </div>
          </div>
        </AnimatedSection>
...
perienza accademica, risultati, onorificenze, attività extracurriculari, ecc."
